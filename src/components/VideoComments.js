...
export const VideoComments = ({ video, setPageLoading, setOpenLoginModal }) => {
  const { user } = useAuth();
  ...
  const { mutate: deleteCommentMutate } = useMutation(deleteComment, {
    ...
    onError: () => {
      toast.success("Unable to delete comment");
      setPageLoading(false);
    },
  });
          ...
          >
            <input
              className={`h-full w-full px-2 outline-none placeholder-white-2 ${
                theme ? "text-white-1 bg-dark-backg" : "text-black-1"
              }`}
              type="text"
            ...
            />
            <button>
              <Send className="text-2xl text-primary-red transition-all lg:hover:text-red-500" />
            </button>
          </form>
        </div>
      </div>
