...
import { liked, watchLater, history } from "../api";
import { SidebarSmall } from "../components/Navbar/SidebarSmall";
...
export const VideoPage = () => {
  ...
  const { isLoading: watchLaterLoading, mutate: watchLaterMutate } =
  ...
  return (
    <div className={`flex ${theme && "bg-dark-backg"}`}>
      ...
      <div className={`w-full lg:w-2/3 ${theme && "bg-dark-backg"}`}>
        {/* Video */}
        ...
        >
          ...
          <div className="flex items-center justify-start mt-5 w-full">
            <button
              ...
              } text-xs xs:text-sm mr-1 xs:mr-2 ${
                likeLoading && "animate-pulse"
              } transition-all`}
              disabled={likeLoading && true}
            >
              <ThumbUpOutlined className="mr-1 w-4 xs:w-5 h-4 xs:h-5" />
              {video?.likeCount}
            </button>
            <button
              onClick={() => {
              ...
              className={`flex items-center tracking-tight lg:tracking-normal font-poppins px-2.5 py-1.5 rounded-3xl ${
                ...
                watchLaterLoading && "animate-pulse"
              } transition-all`}
            >
              <WatchLaterOutlined className="mr-1 w-4 xs:w-5 h-4 xs:h-5" />
              Save to Watch later
            </button>
            <button
              onClick={() => {
            ...
            >
              <PlaylistAdd className="mr-1 w-3.5 xs:w-4 h-3.5 xs:h-4" />
              Save
            </button>
            <button
