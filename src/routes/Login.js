...
export const Login = () => {
  ...
  const handleGuest = () => {
    const data = { email: "guest@gmail.com", password: "guest123" };
    loginMutate(data);
  };

  return (
    <div className={`min-h-screen ${theme && "bg-dark-backg"} lg:pt-6`}>
      <div
        className={`px-5 max-w-screen-sm lg:rounded-lg mx-auto lg:p-8 lg:pt-0 ${
          theme
            ? "bg-dark-backg lg:border lg:border-dark-bor"
          ...
          >
            <div className="mb-4 w-full">
              <input
                className={`border ${
                  theme
                    ? "text-white-1 border-white-1 bg-dark-backg focus:border-gray-2"
            ...
            <button className="bg-primary-red flex justify-center text-white py-3 mt-3 mb-2 w-full cursor-pointer transition-all hover:bg-opacity-90 rounded-md focus:opacity-90">
              {isLoginLoading ? (
                <CircularProgress className="text-2xl animate-spin" />
              ) : (
                <p>Login</p>
              )}
            </button>
